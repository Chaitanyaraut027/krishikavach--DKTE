      ...
      <div className="relative max-w-md w-full">
        ...
        <div className="text-center mb-6">
          <div className="inline-flex flex-col items-center gap-2">
            <div className="w-20 h-20 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-3xl flex items-center justify-center text-5xl shadow-2xl shadow-emerald-300/50">
              🌾
            </div>
        ...
        <div className={`rounded-3xl shadow-2xl overflow-hidden ${isDark ? 'bg-[#0f172a] border border-white/10' : 'bg-white border border-gray-100'}`}>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-7 space-y-5">
            ...
            {/* Mobile Number */}
            <div>
              <label htmlFor="mobileNumber" className={`block text-sm font-bold mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                📱 {t('Mobile Number')}
              </label>
            ...
            {/* Password */}
            <div>
              <label htmlFor="password" className={`block text-sm font-bold mb-2 ${isDark ? 'text-gray-300' : 'text-gray-700'}`}>
                🔒 {t('Password')}
              </label>
              <div className="relative">
        ...
        <div className="mt-6 grid grid-cols-3 gap-3 text-center">
          {[
            ...
            <div key={i} className={`backdrop-blur rounded-2xl py-3 px-2 shadow-sm ${isDark ? 'bg-white/5 border border-white/10' : 'bg-white/70 border border-white'
              ...
              <p className={`text-xs font-semibold ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{t.label}</p>
            </div>
          ))}
        </div>
      </div>
