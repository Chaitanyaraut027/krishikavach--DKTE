...
const FloatingBadge = ({ icon, label, style }) => (
  <div
    className="absolute bg-white/95 backdrop-blur rounded-2xl shadow-xl border border-white/50 px-4 py-3 flex items-center gap-2.5 text-sm font-bold text-gray-800"
      ...
      <section ref={statsRef} className="py-20 relative overflow-hidden" style={isDark ? {} : { background: 'transparent' }}>
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className={`grid grid-cols-2 lg:grid-cols-4 gap-6 ${statsVisible ? 'visible' : ''}`}>
            {[
              { num: c1, suffix: '%+', label: 'Disease Detection Accuracy', icon: '🎯', color: 'from-emerald-500/20 to-teal-500/10 border-emerald-500/30' },
              { num: c2, suffix: '', label: 'AI-Powered Features', icon: '🤖', color: 'from-violet-500/20 to-purple-500/10 border-violet-500/30' },
              { num: c3, suffix: '', label: 'Languages Supported', icon: '🌐', color: 'from-cyan-500/20 to-blue-500/10 border-cyan-500/30' },
              { num: c4, suffix: '/7', label: 'Hours AI Available', icon: '⚡', color: 'from-amber-500/20 to-orange-500/10 border-amber-500/30' },
            ].map((s, i) => (
              <div key={i}
                className={`glass rounded-3xl p-6 text-center bg-gradient-to-br ${s.color} reveal ${statsVisible ? 'visible' : ''}`}
                style={{ animationDelay: `${i * 100}ms` }}>
                <div className="text-3xl mb-2">{s.icon}</div>
                <p className="text-4xl font-black text-white mb-1">
                  {s.num}{s.suffix}
                </p>
                <p className="text-xs text-gray-400 font-semibold leading-tight">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 relative" ref={featuresRef}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {features.map((f, i) => (
              <div key={i}
                className={`feature-card glass rounded-3xl p-6 group cursor-default reveal ${featuresVisible ? 'visible' : ''}`}
              ...
              {roles.map((r, i) => (
                <Link key={r.role} to={r.link}
                  ...
                  <div className="relative p-8">
                    ...
                    {/* Badge & title */}
                    <span className={`inline-block text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full border mb-3 ${r.badge}`}>
                      {r.title}
                    </span>
                    <h3 className="text-2xl font-extrabold text-white mb-3">{r.title}</h3>
                    <p className="text-gray-400 text-sm leading-relaxed mb-6">{r.description}</p>

                    {/* Feature pills */}
                    <div className="flex flex-wrap gap-2 mb-6">
                      {r.features.map((f) => (
                        <span key={f} className="text-xs bg-white/8 text-gray-300 px-3 py-1 rounded-full border border-white/10 font-medium">
                          {f}
                        </span>
                      ))}
                    </div>
