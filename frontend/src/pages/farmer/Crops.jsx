...
const fetchCropImage = async (cropName) => {
  try {
    ...
    // Fallback to Pexels-style URL with crop name
    return `https://source.unsplash.com/800x400/?${encodeURIComponent(cropName + ',farm,crop,agriculture')}`;
  }
};
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-green-50">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex gap-2 mb-7 bg-white rounded-2xl p-1.5 shadow border border-gray-100 fade-up">
          {[
            { id: 'guide', label: `📋 ${t('Crop Guide')}`, icon: '📋' },
            { id: 'mycrops', label: `🌱 ${t('My Crops')} (${myCrops.length})`, icon: '🌱' },
          ].map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`tab-btn flex-1 py-2.5 px-4 rounded-xl font-bold text-sm
                ${activeTab === tab.id
        ...
        {activeTab === 'guide' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6 fade-up">
                ...
                {POPULAR_CROPS.map((crop) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isSelected && <span className="text-[9px] font-bold text-green-600 bg-green-100 px-1.5 py-0.5 rounded-full">✓</span>}
                    </button>
                  );
                })}
              </div>
              ...
              {finalCropName && (
                <div className="mt-3 inline-flex items-center gap-2 bg-emerald-50 border border-emerald-200 px-4 py-1.5 rounded-full text-sm font-semibold text-emerald-800">
                  <span>✅</span> {t('Selected')}: <strong>{finalCropName}</strong>
                </div>
              )}
            </div>
            ...
            >
              {guideLoading ? (
                <>
                  <svg className="spin-slow w-6 h-6" fill="none" viewBox="0 0 24 24">
            ...
            {/* Error */}
            {guideError && (
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 text-red-700 text-sm fade-up">
                ⚠️ {guideError}
              </div>
            )}
              ...
              <div ref={guideRef} className="space-y-5 fade-up">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-gradient-to-br from-emerald-50 to-green-100 border border-emerald-200 rounded-2xl p-5 flex items-center gap-4 shadow">
                    <div className="w-12 h-12 bg-green-600 rounded-2xl flex items-center justify-center text-2xl shadow">💰</div>
                    <div>
                  ...
                  <div className="bg-gradient-to-br from-amber-50 to-yellow-100 border border-amber-200 rounded-2xl p-5 flex items-center gap-4 shadow">
                    <div className="w-12 h-12 bg-amber-500 rounded-2xl flex items-center justify-center text-2xl shadow">📈</div>
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InfoSection icon="🌱" title="Seeds Required" color="bg-lime-50 border-lime-200">
                    <p className="text-sm text-gray-700">{guide.seedsRequired}</p>
                  </InfoSection>
                  <InfoSection icon="🌿" title="Sowing Method & Spacing" color="bg-emerald-50 border-emerald-200">
                    <p className="text-sm text-gray-700 mb-2">{guide.sowingMethod}</p>
                    <p className="text-sm text-gray-700 font-medium">📏 {guide.spacing}</p>
                  </InfoSection>
                </div>

                {/* ── Soil Requirements ────────────────────────────── */}
                <InfoSection icon="🪨" title="Soil Requirements" color="bg-amber-50 border-amber-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                ...
                {/* ── Irrigation ───────────────────────────────────── */}
                <InfoSection icon="💧" title="Irrigation" color="bg-blue-50 border-blue-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                ...
                {/* ── Fertilizers ──────────────────────────────────── */}
                <InfoSection icon="🧪" title="Fertilizers" color="bg-violet-50 border-violet-200">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    {guide.fertilizers?.map((f, i) => (
                      <div key={i} className="bg-white rounded-xl p-3 border border-violet-100 shadow-sm">
                        ...
                        <p className="text-xs text-gray-600 mt-1">📦 {f.quantity}</p>
                        <p className="text-xs text-gray-600">⏱️ {f.timing}</p>
                ...
                <InfoSection icon="📅" title="Growth Stages" color="bg-green-50 border-green-200">
                  <div className="relative">
                    {/* Timeline line */}
                        ...
                        <div key={i} className="relative fade-up">
                          ...
                          <div className="bg-white rounded-xl p-3 border border-green-100 shadow-sm">
                            <div className="flex items-center justify-between mb-1">
                              ...
                              <span className="text-xs bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-medium">{s.duration}</span>
                            </div>
                ...
                <InfoSection icon="🐛" title="Pest & Disease Control" color="bg-red-50 border-red-200">
                  <div className="space-y-3">
                    {guide.pestControl?.map((p, i) => (
                      <div key={i} className="bg-white rounded-xl p-3 border border-red-100 shadow-sm">
                        <p className="font-bold text-red-800 text-sm">⚠️ {p.pest}</p>
                        <p className="text-xs text-gray-600 mt-1">🔍 <strong>Symptoms:</strong> {p.symptoms}</p>
                        <p className="text-xs text-gray-600 mt-0.5">💊 <strong>Remedy:</strong> {p.remedy}</p>
                      </div>
                    ))}
                  </div>
                </InfoSection>
                ...
                {/* ── Harvest ──────────────────────────────────────── */}
                <InfoSection icon="🌾" title="Harvest Information" color="bg-amber-50 border-amber-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <p className="text-xs text-amber-700 font-semibold uppercase mb-1">Duration & Yield</p>
                      <p className="text-sm text-gray-700 mb-1">⏱️ {guide.harvest?.duration}</p>
                      <p className="text-sm font-bold text-green-800">📦 {guide.harvest?.expectedYield}</p>
                    </div>
                    <div>
                ...
                {/* ── Tips & Mistakes ──────────────────────────────── */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <InfoSection icon="💡" title="Success Tips" color="bg-yellow-50 border-yellow-200">
                    <BulletList items={guide.tips} dotColor="bg-yellow-500" />
                  </InfoSection>
                  <InfoSection icon="🚫" title="Common Mistakes to Avoid" color="bg-rose-50 border-rose-200">
                ...
                <div className="text-center py-4 text-xs text-gray-400">
                  ✨ AI-generated crop guide · Specific to {area} {areaUnit} of {guide.cropName} · Consult a local agronomist for precise advice
                </div>
            ...
            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6">
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                  <span>🌱</span> {t('My Crops')}
                </h2>
                <button
                  onClick={() => { setShowAddForm(!showAddForm); setAddError(''); }}
                  className={`px-4 py-2 rounded-xl font-semibold text-sm transition-all shadow
                    ${showAddForm
                      ? 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                      : 'bg-gradient-to-r from-emerald-600 to-green-700 text-white hover:shadow-lg hover:scale-105'}`}
                >
                  {showAddForm ? `✕ ${t('Cancel')}` : `+ ${t('Add Crop')}`}
                </button>
              </div>
              ...
              {showAddForm && (
                <form onSubmit={handleAddCrop} className="space-y-4 border-t border-gray-100 pt-4 fade-up">
                  ...
                  <button type="submit"
                    className="w-full py-3 bg-gradient-to-r from-emerald-600 to-green-700 text-white font-bold rounded-xl hover:shadow-lg transition-all">
                    ✅ Add Crop to My List
                  </button>
                </form>
              )}
            ...
            {loadingCrops ? (
              ...
              <div className="text-center py-16 bg-white rounded-3xl shadow border border-gray-100">
                ...
                <button
                  ...
                  className="mt-4 px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-green-700 text-white text-sm font-bold rounded-xl shadow hover:shadow-lg transition-all"
                >
                  📋 {t('Go to Crop Guide')} →
                </button>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        >
                          <span>📋</span> {t('See Crop Guide')} <span>→</span>
                        </button>
                      </div>
                      <button
                      ...
                      >
                        {deletingId === crop._id ? (
                          <svg className="spin-slow w-4 h-4" fill="none" viewBox="0 0 24 24">
                            <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                            <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
                          </svg>
                        ) : (
